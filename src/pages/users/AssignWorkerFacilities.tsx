import { Link, useParams } from 'react-router-dom'
import type { UserResponse } from '../../entities/Entities'
import { useUserStore } from '../../store/store'
import AssignWorkerFacilitiesForm from '../../components/users/AssignWorkerFacilitiesForm'

export default function AssignWorkerFacilities() {
  const { id } = useParams<{ id: string }>()
  const identifier = id ? parseInt(id, 10) : NaN
  const worker: UserResponse | null = useUserStore((state) => {
    if (!Number.isNaN(identifier)) {
      return state.users.find((u) => u.id === identifier && u.typeId === 1) ?? null
    }
    return null
  })

  if (!worker) {
    return (
      <div className="container max-w-7xl mx-auto py-4 px-3 md:py-5 md:px-4">
        <p className="text-sm text-slate-600">Trabajador no encontrado.</p>
        <Link to="/trabajadores" className="pageBackButton mt-4 inline-block">
          ← Volver a trabajadores
        </Link>
      </div>
    )
  }

  return (
    <div className="container max-w-7xl mx-auto py-4 px-3 md:py-5 md:px-4">
      <div className="mx-auto w-full max-w-2xl">
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <h1 className="mb-0 text-2xl font-bold tracking-tight text-slate-800">
            Asignar instalaciones
          </h1>
          <Link to="/trabajadores" className="pageBackButton shrink-0 self-end sm:self-auto">
            ← Atrás
          </Link>
        </div>
        <AssignWorkerFacilitiesForm worker={worker} />
      </div>
    </div>
  )
}